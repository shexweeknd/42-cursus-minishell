...
void	ft_manage_redirect_file(int fd[2], t_cmd *cmd)
{
	...
	while (tmp_out && tmp_out->next)
		tmp_out = (close(open(tmp_out->args, tmp_out->option, 0777)), \
					tmp_out->next);
	if (tmp_in)
		fd[0] = (close(fd[0]), open(tmp_in->args, tmp_in->option, 0777));
	else
		fd[0] = (close(fd[0]), -1);
	if (tmp_out)
		fd[1] = (close(fd[1]), open(tmp_out->args, tmp_out->option, 0777));
